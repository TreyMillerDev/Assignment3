...
class Worker(Thread):
    ...
    def __init__(self, worker_id, crawl_range, Lock, uniques, total_tokens, num_files):
            ...
            for file in os.listdir(file_path): # this access all the files in the sub directory
                ...
                with open(acc_file, 'r') as json_file:
                    ...
                    if 'content' in data and data['content'].startswith('<'):
                        ...
                        self.num_files += 1 
                        self.total_tokens += len(stemmed_text)
                        for token in stemmed_text:
                            ...
                            if len(self.total) > 2000:
                                print(f'worker no {self.id} full cleared, moving onto next set: ', acc_file, self.num_files)
                                self.total = alpha_sort(self.total)
                                push_to_disk(self.total,self.lock)
                                self.total.clear()
